...
const sortedByGDP = states.sort((a, b) => b.gdp - a.gdp);

console.log(sortedByGDP , "He");
    res.status(200).json(sortedByGDP[0])
});
...
app.put("/states/:id", (req,  res)=>{
    ...
    if(stateIndex == -1){
        res.status(404).json({"Message" : "this state id doesn't belong to our dataBase check twice and then search"})
    }
    states[stateIndex] = {
        id : stateId,
    name: req.body.name,
    ...
    // states.push(state)
    res.status(200).json({"Updated" : states})
})
...
app.put("/states/:id/budget", (req,  res)=>{
    ...
    population: states[stateId].population,
    literacyRate:states[stateId].literacyRate,
    annualBudget:req.body.annualBudget,
     gdp: states[stateId].gdp
    }
    // states.push(state)
    res.status(200).json({"Updated" : states[stateIndex]})
})
app.put("/states/:id/population", (req,  res)=>{
    ...
    population: req.body.population,
    literacyRate:states[stateId].literacyRate,
    annualBudget:states[stateId].annualBudget,
     gdp: states[stateId].gdp
    }
    // states.push(state)
    res.status(200).json({"Updated" : states[stateIndex]})
...
app.patch("/states/:id/literacy", (req,  res)=>{
    const stateId = Number(req.params.id);
    const stateIndex = states.findIndex(u => u.id == stateId);
    console.log(stateIndex , stateId);
...
app.patch("/states/:id", (req,  res)=>{
    const stateId = Number(req.params.id);
    const stateIndex = states.findIndex(u => u.id == stateId);
    console.log(stateIndex , stateId , "index");
...
app.delete("/states/:id", (req , res)=>{
    ...
    res.status(201).json({"Deleted" : states});
})
app.delete("/states/name/:stateName", (req , res)=>{
    const id =req.params.stateName;

    const index = states.findIndex(u => u.name.toLowerCase() == id.toLowerCase());
...
app.delete("/states/low-literacy/:percentage", (req , res)=>{
    const id =Number(req.params.percentage);
for(let i=0; i<states.length ; i++){
    if(states[i].literacyRate < id){
...
app.listen(3000 , ()=>{
    console.log("Server has been started on route localhost:3000");
